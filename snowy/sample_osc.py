...
class SynthesisNet(tf.keras.Model):
    ...
    def call(self, x, c_vec, rnn_1_h, rnn_2_h, init_w, init_kappa):
        ...
        #rnn_2_reshaped = tf.keras.layers.Reshape((1, rnn_2_out.shape[1]))(rnn_2_out)
        output_list.append(rnn_2_out)
...
model = SynthesisNet()
model.load_weights('ffoscillator/checkpoint')
...
def sample(length, input_str=None):
    ...
    for i in range(length - 1):
        ...
        rnn_2_h, w, phi, kappa) = model(x, c_vec, rnn_1_h,
                                                     rnn_2_h,
                                                     w, kappa)
