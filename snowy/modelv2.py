...
class Model():
    def __init__(self, args):
        ...
        self.x = tf.keras.Input(dtype=tf.float32, shape=[args.T, 3])
        self.y =  tf.keras.Input(dtype=tf.float32, shape=[args.T, 3])

        x = tf.split(self.x, args.T, 1)
        x_list = [tf.squeeze(x_i, 1) for x_i in x]
        if args.mode == 'predict':
            self.cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(args.rnn_state_size)
            self.stacked_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([self.cell] * args.num_layers)
        ...
        if args.mode == 'synthesis':
            self.c_vec =  tf.keras.Input(dtype=tf.float32, shape=[args.U, args.c_dimension])
            ...
            for t in range(args.T):
                ...
                #self.phi = tf.reduce_sum(input_tensor=tf.exp(tf.square(-u + self.kappa) * (-beta)) * alpha, axis=1,
                #                    keepdims=True)
                self.phi = tf.reduce_sum(input_tensor=tf.exp(tf.square(-u + self.kappa) * (-beta)) * alpha, axis=1,
                                        keepdims=True)
        ...
        self.output = tf.compat.v1.nn.xw_plus_b(tf.reshape(tf.concat(self.output_list, 1), [-1, args.rnn_state_size]),
                                      output_w, output_b)
        ...
        eps = 1e-20
        self.loss_gaussian = tf.reduce_sum(input_tensor=-tf.math.log(tf.reduce_sum(input_tensor=self.gaussian, axis=1) + eps))
        self.loss_bernoulli = tf.reduce_sum(
            input_tensor=-tf.math.log((self.end_of_stroke + eps) * y_end_of_stroke
